...
\tikzstyle{tensor}=[draw, inner sep=0, minimum size=\tensorsize]
% Used for drawing gaps in the tensor network diagram.
\tikzstyle{notensor}=[minimum size=\tensorsize]
...
\tikzstyle{tensornetwork}=[baseline=-0.25em, xscale=0.75, yscale=0.5, every node/.style={inner sep=2pt},
                          % Add some padding around the picture.
                           execute at end picture={\path (current bounding box.south west) +(-0.1, 0) (current bounding box.north east) +(0.1, 0);}]
